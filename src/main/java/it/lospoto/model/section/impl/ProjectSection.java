...
import it.lospoto.model.entry.impl.ProjectEntry;
...
public class ProjectSection extends GenericSection {
    /* The template filename */
    protected static final String TEMPLATE_FILENAME = "project.vm";
    /* The output filename, which will contain the rendered template and will be stored into result/include folder */
    private static final String LATEX_FILENAME = "project.tex";
     ...
     */
    public ProjectSection(String sectionTitle, String language) {
        /* Parent constructor */
        super(sectionTitle, language);

        /* Create the logger */
        this.logger = FactoryLogger.getInstance().createLogger(ProjectSection.class.getName());
        this.logger.finer("Log for ProjectSection class successfully created");
     ...
     */
    public String toString() { return ProjectSection.class.getName(); }
    ...
    public void build() {
        ...
        this.sectionContent.forEach((entry) -> {
            ...
            SectionEntry sectionEntry = new ProjectEntry(
                    entry.get(ProjectEntry.PROJECT),
                    entry.get(ProjectEntry.COMPANY),
                    entry.get(ProjectEntry.YEAR),
                    entry.get(ProjectEntry.ROLE),
                    entry.get(ProjectEntry.CONTENT)
            );
            ...
            this.logger.finer("Project section entry successfully added to the section list");
        });
    ...
    public void render() {
        ...
        template.addToContext(Constant.CTX_SECTION_PROJECT, this);
        this.logger.finer("Section project successfully added to the context");

        /* Render the section */
        template.render(new File(this.fs.getTemplateResultIncludeSectionsDir(), LATEX_FILENAME));
        this.logger.finer("Section project successfully rendered");
    }
}
