...
import it.lospoto.model.entry.impl.EducationEntry;
...
/**
 * This class implements the education sections.
 */

public class EducationSection extends GenericSection {
    /* The template filename */
    protected static final String TEMPLATE_FILENAME = "education.vm";
    /* The output filename, which will contain the rendered template and will be stored into result/include folder */
    private static final String LATEX_FILENAME = "education.tex";
     ...
     */
    public EducationSection(String sectionTitle, String language) {
        /* Parent constructor */
        super(sectionTitle, language);

        /* Create the logger */
        this.logger = FactoryLogger.getInstance().createLogger(EducationSection.class.getName());
        this.logger.finer("Log for EducationSection class successfully created");
     ...
     */
    public String toString() { return EducationSection.class.getName(); }
    ...
    public void build() {
        ...
        this.sectionContent.forEach((entry) -> {
            ...
            SectionEntry sectionEntry = new EducationEntry(
                    entry.get(EducationEntry.NAME),
                    entry.get(EducationEntry.INSTITUTION),
                    entry.get(EducationEntry.YEAR),
                    entry.get(EducationEntry.CITY),
                    entry.get(EducationEntry.ADVISOR),
                    entry.get(EducationEntry.CONTENT));
            ...
            this.logger.finer("Education section entry successfully added to the section list");
        });
    ...
    public void render() {
        ...
        template.addToContext(Constant.CTX_SECTION_EDUCATION, this);
        this.logger.finer("Section education successfully added to the context");

        /* Render the section */
        template.render(new File(this.fs.getTemplateResultIncludeSectionsDir(), LATEX_FILENAME));
        this.logger.finer("Section education successfully rendered");
    }
}
