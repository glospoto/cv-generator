...
import it.lospoto.model.entry.impl.LanguageEntry;
...
/**
 * This class implements the language sections.
 */

public class LanguageSection extends GenericSection {
    /* The template filename */
    protected static final String TEMPLATE_FILENAME = "language.vm";
    /* The output filename, which will contain the rendered template and will be stored into result/include folder */
    private static final String LATEX_FILENAME = "language.tex";
    ...
    public LanguageSection(String sectionTitle, String language) {
        /* Parent constructor */
        super(sectionTitle, language);

        /* Create the logger */
        this.logger = FactoryLogger.getInstance().createLogger(LanguageSection.class.getName());
        this.logger.finer("Log for LanguageSection class successfully created");
     ...
     */
    public String toString() { return LanguageSection.class.getName(); }
    ...
    public void build() {
        ...
        this.sectionContent.forEach((entry) -> {
            ...
            SectionEntry sectionEntry = new LanguageEntry(
                    entry.get(LanguageEntry.LANGUAGE),
                    entry.get(LanguageEntry.CONTENT)
            );
            ...
            this.logger.finer("Language section entry successfully added to the section list");
        });
    ...
    public void render() {
        ...
        template.addToContext(Constant.CTX_SECTION_LANGUAGE, this);
        this.logger.finer("Section language successfully added to the context");

        /* Render the section */
        template.render(new File(this.fs.getTemplateResultIncludeSectionsDir(), LATEX_FILENAME));
        this.logger.finer("Section language successfully rendered");
    }
}
